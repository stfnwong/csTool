...
for iter = 1:MAX_ITER
    ...
    if(ELLIPTICAL_BOUNDS)
        ...
        if(SAVE_FIGURES)
            ...
            cla(iterax);
            hold(iterax, 'on');
            plot(iterax, X(1,:), X(2,:), 'kx', 'MarkerSize', 8);
            %plot(iterax, w0(1), w0(2), 'rx', 'MarkerSize', 14, 'LineWidth', 2);
            plot(iterax, xel, yel, 'Color', [0 0 1]);
            plot(iterax, wpos(1), wpos(2), 'rx', 'MarkerSize', 16, 'LineWidth', 2);
            %Annotate
            txh = text(wpos(1)+1, wpos(2)+0.4, sprintf('Iteration %d', iter));
            set(txh, 'FontSize', 10, 'FontWeight', 'bold', 'Parent', iterax);
            fn = sprintf('%s%s-iter-%d.tif', PATH, FILENAME, iter);
            title(sprintf('Meanshift Iteration %d', iter));
